...
class BackgroundJobService {
  constructor() {
    ...
    this.concurrencyLimit = 2; // Max 2 concurrent Gemini jobs to avoid rate limiting
    this.processingPromise = null;
    
    // Resume pending jobs on startup (optional, but good for restart resilience)
    this.resumePendingJobs();
  }

  async resumePendingJobs() {
    try {
      // Find jobs that were stuck in 'pending' or 'processing' state during restart
      // Note: We reset 'processing' to 'pending' to retry them, or just process them.
      // For simplicity, we just pick up 'pending' ones.
      const [pendingJobs] = await db.execute(
        "SELECT id FROM background_jobs WHERE status = 'pending' ORDER BY created_at ASC"
      );
      
      if (pendingJobs.length > 0) {
        console.log(`[BackgroundJobService] Resuming ${pendingJobs.length} pending jobs...`);
        for (const job of pendingJobs) {
          this.enqueueJob(job.id);
        }
      }
    } catch (e) {
      console.error('[BackgroundJobService] Error resuming jobs:', e);
    }
  }
  ...
  async createJob(userId, type, data) {
    ...
    this.enqueueJob(result.insertId);
  ...
  async processQueue() {
    // If we are already running max workers, do nothing
    ...
    console.log(`[Queue] Starting job ${jobId}. Active workers: ${this.activeWorkers}/${this.concurrencyLimit}. Queue length: ${this.queue.length}`);
  ...
  async processJob(jobId) {
    try {
      // ATOMIC LOCK: Try to set status to processing ONLY if it is currently pending.
      // This prevents race conditions in cluster mode (multiple PM2 instances).
      const [lockResult] = await db.execute(
        "UPDATE background_jobs SET status = 'processing', progress = 0 WHERE id = ? AND status = 'pending'",
        [jobId]
      );

      // If no rows were affected, it means another worker already picked up this job, or it's not pending.
      if (lockResult.affectedRows === 0) {
        return;
      }

      // Now fetch the job data (we own the lock)
      const [jobs] = await db.execute('SELECT * FROM background_jobs WHERE id = ?', [jobId]);
      if (jobs.length === 0) return;
      const job = jobs[0];

      // Note: We don't need to check job.status here anymore, as the UPDATE above confirmed it was pending.
      // await this.updateJobStatus(jobId, 'processing', 0); // Already done by the lock query
