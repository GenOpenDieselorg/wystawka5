...
router.post('/edit-image', authenticate, upload.single('backgroundImage'), async (req, res) => {
  try {
    ...
    // SECURITY: Validate imageUrl — only allow local upload paths or whitelisted protocols
    // Never pass user-provided URLs directly to axios without validation
    const isLocalPath = imageUrl.startsWith('/uploads/') || imageUrl.startsWith('uploads/');
    const isHttpUrl = imageUrl.startsWith('http://') || imageUrl.startsWith('https://');
    if (!isLocalPath && !isHttpUrl) {
      return res.status(400).json({ error: 'Invalid image URL format' });
    }

    // SECURITY: Validate backgroundImageUrl if provided (same rules)
    if (backgroundImageUrl) {
      const bgIsLocal = backgroundImageUrl.startsWith('/uploads/') || backgroundImageUrl.startsWith('uploads/');
      const bgIsHttp = backgroundImageUrl.startsWith('http://') || backgroundImageUrl.startsWith('https://');
      if (!bgIsLocal && !bgIsHttp) {
        return res.status(400).json({ error: 'Invalid background image URL format' });
      }
    }
